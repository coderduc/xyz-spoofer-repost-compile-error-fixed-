...
void ProcessKiller() {
    if (IsProcessRunning((_xor_(L"steam.exe")).c_str()))
        system((_xor_("taskkill /IM steam.exe /T /F")).c_str());

    if (IsProcessRunning((_xor_(L"steamwebhelper.exe")).c_str()))
        system((_xor_("taskkill /IM steamwebhelper.exe /T /F")).c_str());

    if (IsProcessRunning((_xor_(L"SteamService.exe")).c_str()))
        system((_xor_("taskkill /IM SteamService.exe /T /F")).c_str());

    if (IsProcessRunning((_xor_(L"Origin.exe")).c_str()))
        system((_xor_("taskkill /IM Origin.exe /T /F")).c_str());

    if (IsProcessRunning((_xor_(L"OriginWebHelperService.exe")).c_str()))
        system((_xor_("taskkill /IM OriginWebHelperService.exe /T /F")).c_str());

    if (IsProcessRunning((_xor_(L"EpicGamesLauncher.exe")).c_str()))
        system((_xor_("taskkill /IM EpicGamesLauncher.exe /T /F")).c_str());

    if (IsProcessRunning((_xor_(L"RiotClientServices.exe")).c_str()))
        system((_xor_("taskkill /IM RiotClientServices.exe /T /F")).c_str());

    if (IsProcessRunning((_xor_(L"RiotClientUx.exe")).c_str()))
        system((_xor_("taskkill /IM RiotClientUx.exe /T /F")).c_str());

    if (IsProcessRunning((_xor_(L"RiotClientUxRender.exe")).c_str()))
        system((_xor_("taskkill /IM RiotClientUxRender.exe /T /F")).c_str());

    if (IsProcessRunning((_xor_(L"RiotClientCrashHandler.exe")).c_str()))
        system((_xor_("taskkill /IM RiotClientCrashHandler.exe /T /F")).c_str());

    if (IsProcessRunning((_xor_(L"FortniteClient-Win64-Shipping.exe")).c_str()))
        system((_xor_("taskkill /IM FortniteClient-Win64-Shipping.exe /T /F")).c_str());

    if (IsProcessRunning((_xor_(L"OneDrive.exe")).c_str()))
        system((_xor_("taskkill /IM OneDrive.exe /T /F")).c_str());

    if (IsProcessRunning((_xor_(L"RustClient.exe")).c_str()))
        system((_xor_("taskkill /IM RustClient.exe /T /F")).c_str());

    if (IsProcessRunning((_xor_(L"r5apex.exe")).c_str()))
        system((_xor_("taskkill /IM r5apex.exe /T /F")).c_str());

    if (IsProcessRunning((_xor_(L"vgtray.exe")).c_str()))
...
void GayMacChanger() {
    ...
    if (FileExists(changer)) {
        ...
        DeleteFile((LPCWSTR)changer.c_str());
    }
}
void DriverLoad() {
        ...
        if (FileExists(gdrvRandomize)) {
            if (FileExists(mapperRandomize)) {
                if (FileExists(driverRandomize)) {
                    ...
                    DeleteFile((LPCWSTR)mapperRandomize.c_str());
                    DeleteFile((LPCWSTR)gdrvRandomize.c_str());
                    DeleteFile((LPCWSTR)driverRandomize.c_str());
                    DeleteFile((LPCWSTR)finalPath.c_str());
                }
            }
        }
    }
